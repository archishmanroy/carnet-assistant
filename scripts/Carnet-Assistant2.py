...
def setupArgumentParser():
    parser = argparse.ArgumentParser(
        description='provide name of google worksheet (not filename!)',
...
if __name__ == "__main__":
    ...
    path_pdf_templates = path.join(basedir, "templates")
    ...
    fdf_footer = "\n] >> >>\nendobj\ntrailer\n<</Root 1 0 R>>\n%%EOF"
    data_keys = {"item": "A", "description": "B", "quantity": "C", "mass": "D", "value": "E", "country": "F",
                 "image": "Z"}
    unit = {"C": "", "D": " kg", "E": " EUR"}
    ...
    for page_number in range(npages + 1):
        ...
        with open("{0}/page-{1}.fdf".format(path_fdf_dir, page_number), "w+") as file:
            ...
            for key in page_sums[page_number + 1]:
                ...
                sums_key_value_pair = "<< /T(S.{0})/V({1}{2}) >>\n".format(key,
                                                                           float(page_sums[page_number + 1][key]) / 100,
                                                                           unit[key])
        ...
        #############################################################
        fill_form_in_bash = "pdftk {0}/{1} fill_form {2}/page-{4}.fdf output {3}/page-{4}.pdf flatten".format(
            path_pdf_templates, extra_templates[page_number % 2], path_fdf_dir, page_output_dir, page_number)
    ...
    for index in range(len(front_page_templates)):
        ...
        system("pdftk {0} {1} cat output {2}".format(current_front_page, str_finished_list,
                                                     path.join(carnet_output_dir, outputs[index])))
