...
def checkImages(data, containingDirectory):
    ...
    if len(duplicates_set) > 0 or len(data) != len(files) or len(missing_list) > 0:
        ...
        if len(duplicates_set) > 0:
            print(
                        'ERROR Filenames must be unique. Dont forget to update spreadsheet if you change one. \n\tDuplicated files: ' + ', '.join(
                    duplicates_set))
        print(len(data))
        print(len(files))
        if (len(data) < len(files)):
            print("ERROR Did you take too many selfies?")
            print("\tThe spreedshet lacks {} image/s.".format(abs(len(data) - len(listdir(containingDirectory)))))
            print("\tImages missing from the spreadsheet: {}".format(', '.join(missing_dir)))

        if (len(data) > len(files)):
            print("ERROR Did you put too many entries?")
            print("\tThe image folder lacks {} image/s.".format(abs(len(data) - len(listdir(containingDirectory)))))
...
def assembleImages(basedir, sheet, out, archive_dir, font, labeltype):
    ...
    image_w, image_h = int((w - (lmargin + rmargin)) / (n_w + (n_w - 1) * r_hgap)), int(
        (h - (footer + header)) / (n_h + (n_h - 1) * r_vgap))
    hgap, vgap = int((w - (lmargin + rmargin) - n_w * image_w) / (n_w - 1)), int(
        (h - (footer + header) - n_h * image_h) / (n_h - 1))

    if image_w * n_w + hgap * (n_w - 1) + lmargin + rmargin > w or image_h * n_h + vgap * (
            n_h - 1) + footer + header > h:
    ...
    for n_page in range(n_pages + 1):
        ...
        for file in range(n_w * n_h):
            ...
            if not path.isfile(path.join(postprocessed_images_dir, "_{}.jpeg".format(n_page * n_h * n_w + file + 1))):
                break
            pos = (int(lmargin + ((file * image_w + (file - file / n_w) * hgap) % (w - (lmargin + rmargin)))),
                   header + file // n_w * (image_h + vgap))
            # print(", ".join(file, header, file, n_w, image_h, vgap))
            page.paste(
                Image.open(path.join(postprocessed_images_dir, "_{}.jpeg".format(n_page * n_h * n_w + file + 1))),
                box=pos)
